/*!
 * \file irnss_l5_telemetry_decoder_gs.cc
...
#include "irnss_utc_model.h"         // for Irnss_Utc_Model
#include "tlm_utils.h"
...
#include <bit>
namespace irnss_rotl = std;
#else
namespace irnss_rotl
{
...
irnss_l5_telemetry_decoder_gs_sptr
irnss_l5_make_telemetry_decoder_gs(const Gnss_Satellite &satellite, const Tlm_Conf &conf)
{
    return irnss_l5_telemetry_decoder_gs_sptr(new irnss_l5_telemetry_decoder_gs(satellite, conf));
}


irnss_l5_telemetry_decoder_gs::irnss_l5_telemetry_decoder_gs(
    const Gnss_Satellite &satellite,
    const Tlm_Conf &conf) : gr::block("irnss_navigation_gs", gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)),
                                gr::io_signature::make(1, 1, sizeof(Gnss_Synchro))),
                            d_dump_filename(conf.dump_filename),
                            d_sample_counter(0ULL),
                            d_preamble_index(0ULL),
                            d_last_valid_preamble(0),
                            d_bits_per_preamble(IRNSS_PREAMBLE_LENGTH_BITS),
                            d_samples_per_preamble(IRNSS_PREAMBLE_LENGTH_BITS),
                            d_preamble_period_symbols(IRNSS_SUBFRAME_BITS),
                            d_CRC_error_counter(0),
                            d_channel(0),
                            d_required_symbols(IRNSS_SUBFRAME_BITS),
                            d_prev_IRNSS_frame_4bytes(0),
                            d_stat(0),
                            ...
                            d_dump_crc_stats(conf.dump_crc_stats)
{
    ...
    DLOG(INFO) << "Initializing IRNSS L5 TELEMETRY DECODER";
...
irnss_l5_telemetry_decoder_gs::~irnss_l5_telemetry_decoder_gs()
{
    DLOG(INFO) << "IRNSS L5 Telemetry decoder block (channel " << d_channel << ") destructor called.";
    size_t pos = 0;
...
bool irnss_l5_telemetry_decoder_gs::irnss_word_parityCheck(uint32_t irnssword)
{
    // XOR as many bits in parallel as possible.  The magic constants pick
    //   up bits which are to be XOR'ed together to implement the IRNSS parity
    //   check algorithm described in IS-IRNSS-200M.  This avoids lengthy shift-
    //   and-xor loops.
    const uint32_t d1 = irnssword & 0xFBFFBF00U;
    const uint32_t d2 = irnss_rotl::rotl(irnssword, 1U) & 0x07FFBF01U;
    const uint32_t d3 = irnss_rotl::rotl(irnssword, 2U) & 0xFC0F8100U;
    const uint32_t d4 = irnss_rotl::rotl(irnssword, 3U) & 0xF81FFE02U;
    const uint32_t d5 = irnss_rotl::rotl(irnssword, 4U) & 0xFC00000EU;
    const uint32_t d6 = irnss_rotl::rotl(irnssword, 5U) & 0x07F00001U;
    const uint32_t d7 = irnss_rotl::rotl(irnssword, 6U) & 0x00003000U;
    const uint32_t t = d1 ^ d2 ^ d3 ^ d4 ^ d5 ^ d6 ^ d7;
    // Now XOR the 5 6-bit fields together to produce the 6-bit final result.
    uint32_t parity = t ^ irnss_rotl::rotl(t, 6U) ^ irnss_rotl::rotl(t, 12U) ^ irnss_rotl::rotl(t, 18U) ^ irnss_rotl::rotl(t, 24U);
    parity = parity & 0x3FU;
    if (parity == (irnssword & 0x3FU))
        {
            return true;
        }
...
void irnss_l5_telemetry_decoder_gs::set_satellite(const Gnss_Satellite &satellite)
{
...
void irnss_l5_telemetry_decoder_gs::set_channel(int32_t channel)
{
    d_channel = channel;
...
bool irnss_l5_telemetry_decoder_gs::decode_subframe(bool flag_invert)
{
    std::array<char, IRNSS_SUBFRAME_LENGTH> subframe{};
    int32_t frame_bit_index = 0;
    int32_t word_index = 0;
    uint32_t IRNSS_frame_4bytes = 0;
            ...
            if (flag_invert == false)
                {
                    if (subframe_symbol > 0)
                        {
                            IRNSS_frame_4bytes += 1;  // insert the telemetry bit in LSB
                        }
                }
            else
                {
                    if (subframe_symbol < 0)
                        {
                            IRNSS_frame_4bytes += 1;  // insert the inverted telemetry bit in LSB
                        }
                }
            // ******* bits to words ******
            frame_bit_index++;
            if (frame_bit_index == 30)
                {
                    ...
                    //      Bits 0 to 29 = the IRNSS data word
                    //      Bits 30 to 31 = 2 LSBs of the IRNSS word ahead.
                    // prepare the extended frame [-2 -1 0 ... 30]
                    if (d_prev_IRNSS_frame_4bytes & 0x00000001U)
                        {
                            IRNSS_frame_4bytes = IRNSS_frame_4bytes | 0x40000000U;
                        }
                    if (d_prev_IRNSS_frame_4bytes & 0x00000002U)
                        {
                            IRNSS_frame_4bytes = IRNSS_frame_4bytes | 0x80000000U;
                        }
                    // Check that the 2 most recently logged words pass parity. Have to first
                    // invert the data bits according to bit 30 of the previous word.
                    if (IRNSS_frame_4bytes & 0x40000000U)
                        {
                            IRNSS_frame_4bytes ^= 0x3FFFFFC0U;  // invert the data bits (using XOR)
                        }
                    // check parity. If ANY word inside the subframe fails the parity, set subframe_synchro_confirmation = false
                    bool crc_ok = irnss_l5_telemetry_decoder_gs::irnss_word_parityCheck(IRNSS_frame_4bytes);
                    if (d_dump_crc_stats)
                        {
                    ...
                    std::memcpy(&subframe[word_index * IRNSS_WORD_LENGTH], &IRNSS_frame_4bytes, sizeof(uint32_t));
                    word_index++;
                    d_prev_IRNSS_frame_4bytes = IRNSS_frame_4bytes;  // save the actual frame
                    IRNSS_frame_4bytes = 0;
                }
            else
                {
                    IRNSS_frame_4bytes <<= 1U;  // shift 1 bit left the telemetry word
                }
        }

    // decode subframe
    // NEW IRNSS SUBFRAME HAS ARRIVED!
    if (subframe_synchro_confirmation)
        {
            if (d_enable_navdata_monitor)
                {
                    uint32_t irnss_word;
                    std::bitset<IRNSS_SUBFRAME_BITS> subframe_bits;
...
void irnss_l5_telemetry_decoder_gs::reset()
{
...
int irnss_l5_telemetry_decoder_gs::general_work(int noutput_items __attribute__((unused)), gr_vector_int &ninput_items __attribute__((unused)),
    ...
    if (d_symbol_history.empty())
        {
            ...
            for (int32_t i = 0; i < IRNSS_PREAMBLE_LENGTH_BITS; i++)
                {
                ...
                if (abs(corr_value) >= d_samples_per_preamble)
                    {
                        ...
                        DLOG(INFO) << "Preamble detection for IRNSS L5 satellite " << this->d_satellite;
                        d_prev_IRNSS_frame_4bytes = 0;
    ...
    if (d_flag_preamble == true)
        {
            if (!(d_nav.get_TOW() == 0))
                ...
                {
                    DLOG(INFO) << "Received IRNSS L5 TOW equal to zero at sat " << d_nav.get_satellite_PRN();
                }
        }
    else
        {
            if (d_flag_TOW_set == true)
                {
                    d_TOW_at_current_symbol_ms += IRNSS_L5_BIT_PERIOD_MS;
                }
        }
