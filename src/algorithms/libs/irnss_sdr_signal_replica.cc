/*!
 * \file irnss_sdr_signal_replica.cc
...
void irnss_l5_code_gen_float(own::span<float> dest, int32_t prn, uint32_t chip_shift)
{
    ...
    irnss_l5_code_gen_int(ca_code_int, prn, chip_shift);
...
/*
 *  Generates complex IRNSS L5 code for the desired SV ID and sampled to specific sampling frequency
...
void irnss_l5_code_gen_complex_sampled(own::span<std::complex<float>> dest, uint32_t prn, int32_t sampling_freq, uint32_t chip_shift)
{
    ...
    irnss_l5_code_gen_complex(code_aux, prn, chip_shift);  // generate C/A code 1 sample per chip
