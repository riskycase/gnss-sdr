...
/*!
 * \brief This class decodes a IRNSS NAV Data message as described in IS-IRNSS-200M
 *
...
public:
     ...
     */
    Irnss_Ephemeris get_ephemeris() const;
     ...
     * returns it in [s] (IS-IRNSS-200M, 20.3.3.5.2.4)
     */
    double utc_time(double irnsstime_corrected) const;
     ...
     */
    inline int32_t get_IRNSS_week() const
    {
        return i_IRNSS_week;
    }
...
private:
    uint64_t read_navigation_unsigned(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const;
    int64_t read_navigation_signed(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const;
    bool read_navigation_bool(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const;
    void print_irnss_word_bytes(uint32_t IRNSS_word) const;
    ...
    // broadcast orbit 1
    int32_t d_TOW{};      // Time of IRNSS Week of the ephemeris set (taken from subframes TOW) [s]
    ...
    int32_t d_TOW_SF5{};  // Time of IRNSS Week from HOW word of Subframe 5 [s]
    int32_t d_IODE_SF2{};
    int32_t d_IODE_SF3{};
    ...
    // broadcast orbit 3
    int32_t d_Toe{};    // Ephemeris data reference time of week (Ref. 20.3.3.4.3 IS-IRNSS-200M) [s]
    int32_t d_Toc{};    // clock data reference time (Ref. 20.3.3.3.3.1 IS-IRNSS-200M) [s]
    ...
    // broadcast orbit 6
    int32_t i_SV_accuracy{};  // User Range Accuracy (URA) index of the SV (reference paragraph 6.2.1) for the standard positioning service user (Ref 20.3.3.3.1.3 IS-IRNSS-200M)
    int32_t i_SV_health{};
    ...
    int32_t i_WN_A{};  // Modulo 256 of the IRNSS week number to which the almanac reference time (i_Toa) is referenced
    ...
    // UTC parameters
    double d_A0{};  // Constant of a model that relates IRNSS and UTC time (ref. 20.3.3.5.2.4 IS-IRNSS-200M) [s]
    double d_A1{};  // 1st order term of a model that relates IRNSS and UTC time (ref. 20.3.3.5.2.4 IS-IRNSS-200M) [s/s]

    int32_t d_t_OT{};        // Reference time for UTC data (reference 20.3.4.5 and 20.3.3.5.2.4 IS-IRNSS-200M) [s]
...
/** \} */
/** \} */
#endif  // GNSS_SDR_IRNSS_NAVIGATION_MESSAGE_H
