/*!
 * \file irnss_navigation_message.cc
 * \brief  Implementation of a IRNSS NAV Data message decoder as described in IS-IRNSS-200M
 * \author Javier Arribas, 2011. jarribas(at)cttc.es
 *
 * See https://www.irnss.gov/technical/icwg/IS-IRNSS-200M.pdf Appendix II
 *
 *
 * -----------------------------------------------------------------------------
 *
...
Irnss_Navigation_Message::Irnss_Navigation_Message()
{
    ...
    const std::string _system("IRNSS");
...
void Irnss_Navigation_Message::print_irnss_word_bytes(uint32_t IRNSS_word) const
{
    std::cout << " Word =" << std::bitset<32>(IRNSS_word) << '\n';
}


bool Irnss_Navigation_Message::read_navigation_bool(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    bool value;

    if (static_cast<int>(bits[IRNSS_SUBFRAME_BITS - parameter[0].first]) == 1)
        {
            value = true;
        }
    else
...
uint64_t Irnss_Navigation_Message::read_navigation_unsigned(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    uint64_t value = 0ULL;
    ...
    for (int32_t i = 0; i < num_of_slices; i++)
        {
            for (int32_t j = 0; j < parameter[i].second; j++)
                {
                    value <<= 1;  // shift left
                    if (static_cast<int>(bits[IRNSS_SUBFRAME_BITS - parameter[i].first - j]) == 1)
                        {
...
int64_t Irnss_Navigation_Message::read_navigation_signed(const std::bitset<IRNSS_SUBFRAME_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    int64_t value = 0LL;
    const int32_t num_of_slices = parameter.size();
...
int32_t Irnss_Navigation_Message::subframe_decoder(char* subframe)
{
    uint32_t irnss_word;
    ...
    switch (subframe_ID)
        ...
        // ICD (IS-IRNSS-200M Appendix II). https://www.irnss.gov/technical/icwg/IS-IRNSS-200M.pdf
        case 1:
            // --- It is subframe 1 -------------------------------------
        ...
        case 4:  // --- It is subframe 4 ---------- Almanac, ionospheric model, UTC parameters, SV health (PRN: 25-32)
            ...
            if (SV_page > 24 && SV_page < 33)  // Page 4 (from Table 20-V. Data IDs and SV IDs in Subframes 4 and 5, IS-IRNSS-200M)
                {
                    //! \TODO read almanac
                    if (SV_data_ID != 0)
                        {
            ...
            if (SV_page == 52)  // Page 13 (from Table 20-V. Data IDs and SV IDs in Subframes 4 and 5, IS-IRNSS-200M)
                {
                    //! \TODO read Estimated Range Deviation (ERD) values
                }

            if (SV_page == 56)  // Page 18 (from Table 20-V. Data IDs and SV IDs in Subframes 4 and 5, IS-IRNSS-200M)
                {
            ...
            if (SV_page == 63)  // Page 25 (from Table 20-V. Data IDs and SV IDs in Subframes 4 and 5, IS-IRNSS-200M)
                {
        ...
        case 5:  // -- It is subframe 5 -----------------almanac health (PRN: 1-24) and Almanac reference week number and time.
            ...
            if (SV_page_5 == 51)  // Page 25 (from Table 20-V. Data IDs and SV IDs in Subframes 4 and 5, IS-IRNSS-200M)
                {
...
double Irnss_Navigation_Message::utc_time(double irnsstime_corrected) const
{
    double t_utc;
    ...
    const int32_t weeksToLeapSecondEvent = i_WN_LSF - i_IRNSS_week;
            ...
            else  // we are in the same week than the leap second event
                {
                    if (std::abs(irnsstime_corrected - secondOfLeapSecondEvent) > 21600)
                    ...
                    else
                        {
                             ...
                             */
                            const int32_t W = static_cast<int32_t>(fmod(irnsstime_corrected - Delta_t_UTC - 43200, 86400)) + 43200;
    ...
    const double secondsOfWeekBeforeToday = 43200 * floor(irnsstime_corrected / 43200);
    t_utc = secondsOfWeekBeforeToday + t_utc_daytime;
    return t_utc;
}


Irnss_Ephemeris Irnss_Navigation_Message::get_ephemeris() const
{
