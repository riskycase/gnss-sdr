...
/*!
 * \brief This class is a storage for the IRNSS UTC MODEL data as described in IS-IRNSS-200M
 *
...
public:
     ...
     * Default constructor
     */
    Irnss_Utc_Model() = default;

    // UTC parameters
    double A0{};           //!< Constant of a model that relates IRNSS and UTC time (ref. 20.3.3.5.2.4 IS-IRNSS-200M) [s]
    double A1{};           //!< 1st order term of a model that relates IRNSS and UTC time (ref. 20.3.3.5.2.4 IS-IRNSS-200M) [s/s]
    double A2{};           //!< 2nd order term of a model that relates IRNSS and UTC time (ref. 20.3.3.5.2.4 IS-IRNSS-200M) [s/s]
    int32_t tot{};         //!< Reference time for UTC data (reference 20.3.4.5 and 20.3.3.5.2.4 IS-IRNSS-200M) [s]
    int32_t WN_T{};        //!< UTC reference week number [weeks]
    int32_t DeltaT_LS{};   //!< Delta time due to leap seconds [s]. Number of leap seconds since 6-Jan-1980 as transmitted by the IRNSS almanac.
...
/** \} */
/** \} */
#endif  // GNSS_SDR_IRNSS_UTC_MODEL_H
