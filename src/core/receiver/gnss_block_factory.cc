...
#include "irnss_l5_telemetry_decoder.h"
...
std::unique_ptr<GNSSBlockInterface> GNSSBlockFactory::GetObservables(const ConfigurationInterface* configuration)
{
    ...
    unsigned int IRNSS_channels = configuration->property("Channels_I5.count", 0);
    ...
    try
        {
            ...
            else if (implementation == "IRNSS_L5_PCPS_Acquisition")
                {
                    std::unique_ptr<AcquisitionInterface> block_ = std::make_unique<IrnssL5PcpsAcquisition>(configuration, role, in_streams,
                        out_streams);
                    block = std::move(block_);
                }
#if OPENCL_BLOCKS
            ...
            else if (implementation == "IRNSS_L5_DLL_PLL_Tracking")
                {
                    std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<IrnssL5DllPllTracking>(configuration, role, in_streams,
                        out_streams);
                    block = std::move(block_);
                }
#if CUDA_GPU_ACCEL
            ...
            else if (implementation == "IRNSS_L5_Telemetry_Decoder")
                {
                    std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<IrnssL5TelemetryDecoder>(configuration, role, in_streams,
                        out_streams);
                    block = std::move(block_);
                }
    ...
    unsigned int out_streams)
{
    ...
    if (implementation == "GPS_L1_CA_DLL_PLL_Tracking")
        ...
        {
            std::unique_ptr<TrackingInterface> block_ = std::make_unique<IrnssL5DllPllTracking>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
#if CUDA_GPU_ACCEL
    ...
    unsigned int out_streams)
{
    ...
    if (implementation == "GPS_L1_CA_Telemetry_Decoder")
        ...
        {
            std::unique_ptr<TelemetryDecoderInterface> block_ = std::make_unique<IrnssL5TelemetryDecoder>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
